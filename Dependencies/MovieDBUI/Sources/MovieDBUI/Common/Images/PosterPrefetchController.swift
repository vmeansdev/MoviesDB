...
@MainActor
public protocol PosterPrefetchControlling: Sendable {
...
@MainActor
public final class PosterPrefetchController: PosterPrefetchControlling, @unchecked Sendable {
    private let posterImagePrefetcher: any PosterImagePrefetching

    private var visibleIndices = Set<Int>()
    private var prefetchTask: Task<Void, Never>?

    public init(posterImagePrefetcher: any PosterImagePrefetching) {
        self.posterImagePrefetcher = posterImagePrefetcher
    }

    public func itemVisibilityChanged(index: Int, isVisible: Bool, columns: Int, itemCount: Int, posterURLAt: @escaping (Int) -> URL?) {
        if isVisible {
    ...
    public func itemCountChanged(columns: Int, itemCount: Int, posterURLAt: @escaping (Int) -> URL?) {
        schedulePrefetch(columns: columns, itemCount: itemCount, posterURLAt: posterURLAt)
    }
